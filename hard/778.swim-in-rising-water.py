...
class Solution:
    def swimInWater(self, grid: List[List[int]]) -> int:
        ...
        while minpq:
            t,r,c=heapq.heappop(minpq)
            #! t is the time/height 
            #! r and c is row and column 
            if r==n-1 and c ==n-1:
                return t
            for direction in directions:
                neigR=direction[0]+r
                neigC=direction[1]+c
                if(neigR<0 or neigC<0 or neigR==n or neigC==n or (neigR,neigC) in visited ):
                    continue
                heapq.heappush(minpq,[max(t,grid[neigR][neigC]),neigR,neigC])
